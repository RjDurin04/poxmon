...
export default async function ItemAttributeDetailPage({ params }: PageProps) {
    const { slug } = await params;
    const attr = await getItemAttribute(slug);
    const englishName = attr.names.find((n) => n.language.name === "en")?.name ?? attr.name;
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                        <div>
                            ...
                            <p className="text-text-muted mt-4 max-w-2xl text-lg font-medium leading-relaxed italic">
                                &quot;{attr.descriptions.find(d => d.language.name === "en")?.description}&quot;
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="lg:col-span-12">
                        <div className="flex items-center justify-between mb-8">
                            <div>
                                ...
                                <p className="text-text-muted text-sm mt-1">Artifacts sharing the {englishName.toLowerCase()} property</p>
                            </div>
                        </div>
