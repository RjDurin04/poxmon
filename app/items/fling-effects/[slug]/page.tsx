import { getItemFlingEffect } from "@/lib/api";
import Link from "next/link";
import Image from "next/image";
import { Zap, ArrowLeft, Disc } from "lucide-react";

interface PageProps {
    params: Promise<{ slug: string }>;
}

export default async function ItemFlingEffectDetailPage({ params }: PageProps) {
    const { slug } = await params;
    const effect = await getItemFlingEffect(slug);
            ...
            <div className="relative overflow-hidden pt-24 pb-20 px-4 md:px-8 border-b border-border">
                ...
                <div className="relative max-w-7xl mx-auto">
                    ...
                    >
                        ...
                        <span className="text-sm font-black uppercase tracking-widest">Back to Items</span>
                    </Link>
            ...
            <div className="max-w-7xl mx-auto px-4 md:px-8 py-16">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                    <div className="lg:col-span-12">
                        <div className="bg-bg-secondary rounded-[32px] p-8 border border-border mb-12">
                            <div className="flex items-center gap-3 mb-6">
                                <Disc className="w-5 h-5 text-accent" />
                                <h2 className="text-sm font-black uppercase tracking-widest text-text-primary">Fling Effect</h2>
                            </div>
                            <p className="text-text-primary text-xl leading-relaxed italic">
                                &quot;{effect.effect_entries.find(e => e.language.name === "en")?.effect}&quot;
                            </p>
                        </div>
                        ...
                        <div className="flex items-center justify-between mb-8">
                            <div>
                                ...
                                <p className="text-text-muted text-sm mt-1">Items that trigger this specific effect upon impact</p>
                            </div>
                        </div>
