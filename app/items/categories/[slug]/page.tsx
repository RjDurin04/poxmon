import { getItemCategory } from "@/lib/api";
import Link from "next/link";
import { Package, ArrowLeft, Disc } from "lucide-react";

interface PageProps {
    params: Promise<{ slug: string }>;
}

export default async function ItemCategoryDetailPage({ params }: PageProps) {
    const { slug } = await params;
    const category = await getItemCategory(slug);
    const englishName = category.names.find((n) => n.language.name === "en")?.name ?? category.name;
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                        <div>
                            ...
                            <p className="text-text-muted mt-4 max-w-2xl text-lg font-medium leading-relaxed italic">
                                Logistic classification for items within the {category.pocket.name.replace(/-/g, " ")} inventory sector.
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="lg:col-span-12">
                        <div className="flex items-center justify-between mb-8">
                            <div>
                                ...
                                <p className="text-text-muted text-sm mt-1">Tools and artifacts belonging to this category</p>
                            </div>
                        </div>
