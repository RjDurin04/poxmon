...
export default async function BerryFirmnessDetailPage({ params }: PageProps) {
    const { slug } = await params;
    const firmness = await getBerryFirmness(slug);
    const englishName = firmness.names.find((n) => n.language.name === "en")?.name ?? firmness.name;
                        ...
                        <div>
                            <div className="flex items-center gap-3 mb-4">
                                <div className="w-12 h-12 rounded-2xl bg-accent/20 flex items-center justify-center border border-accent/30 shadow-lg shadow-accent/5">
                                    <Carrot className="w-6 h-6 text-accent" />
                                </div>
                            ...
                            <p className="text-text-muted mt-4 max-w-2xl text-lg font-medium leading-relaxed italic">
                                Species of berries categorized by their {englishName.toLowerCase()} physical texture.
                            </p>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="flex items-center justify-between mb-8">
                            <div>
                                <h2 className="text-2xl font-black text-text-primary flex items-center gap-3">
                                    Berries in this Class
                                    <span className="text-sm font-bold px-2 py-0.5 bg-accent/10 text-accent rounded-lg border border-accent/20">
                                        {firmness.berries.length}
                                    </span>
                                </h2>
                                <p className="text-text-muted text-sm mt-1">Berries with {englishName.toLowerCase()} texture</p>
                            </div>
                        </div>
