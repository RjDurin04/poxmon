...
export default async function MoveTargetDetailPage({ params }: PageProps) {
    const { slug } = await params;
    const target = await getMoveTarget(slug);
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                        <div>
                            ...
                            <p className="text-text-muted mt-4 max-w-2xl text-lg font-medium leading-relaxed italic">
                                &quot;{target.descriptions.find(d => d.language.name === "en")?.description}&quot;
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="lg:col-span-12">
                        <div className="flex items-center justify-between mb-8">
                            <div>
                                ...
                                <p className="text-text-muted text-sm mt-1">Techniques that use {target.name.replace(/-/g, " ")} as a move target</p>
                            </div>
                        </div>
