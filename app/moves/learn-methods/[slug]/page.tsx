...
export default async function MoveLearnMethodDetailPage({ params }: PageProps) {
    const { slug } = await params;
    const method = await getMoveLearnMethod(slug);
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                        <div>
                            ...
                            <p className="text-text-muted mt-4 max-w-2xl text-lg font-medium leading-relaxed italic">
                                &quot;{method.descriptions.find(d => d.language.name === "en")?.description}&quot;
                            </p>
                        </div>
                    </div>
                </div>
                            ...
                            <div>
                                <h2 className="text-2xl font-black text-text-primary flex items-center gap-3">
                                    ...
                                    <span className="text-sm font-bold px-2 py-0.5 bg-accent/10 text-accent rounded-lg border border-accent/20">
                                        {method.version_groups.length}
                                    </span>
                                </h2>
                                <p className="text-text-muted text-sm mt-1">Generations where this learning method is active</p>
                            </div>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {method.version_groups.map((group) => (
                                <Link
                                    key={group.name}
                                    href={`/games/version-groups/${group.name}`}
                                ...
                                >
                                    <div className="w-12 h-12 rounded-2xl bg-bg-tertiary flex items-center justify-center group-hover:bg-accent/10 transition-colors">
                                        <Gamepad2 className="w-6 h-6 text-text-muted group-hover:text-accent transition-colors" />
                                    </div>
                                    <div>
                                        <div className="text-[10px] font-black text-text-muted uppercase tracking-widest">Version Group</div>
