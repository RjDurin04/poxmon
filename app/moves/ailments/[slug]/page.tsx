import { getMoveAilment } from "@/lib/api";
import Link from "next/link";
import { Skull, ArrowLeft, Activity } from "lucide-react";

interface PageProps {
    params: Promise<{ slug: string }>;
}

export default async function MoveAilmentDetailPage({ params }: PageProps) {
    const { slug } = await params;
    const ailment = await getMoveAilment(slug);
    const englishName = ailment.names.find((n) => n.language.name === "en")?.name ?? ailment.name;
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                        <div>
                            ...
                            <p className="text-text-muted mt-4 max-w-2xl text-lg font-medium leading-relaxed italic">
                                Negative status condition documentation affecting biological performance and turn-order priority.
                            </p>
                        </div>
                    </div>
                </div>
                            ...
                            <div>
                                <h2 className="text-2xl font-black text-text-primary flex items-center gap-3">
                                    ...
                                    <span className="text-sm font-bold px-2 py-0.5 bg-accent/10 text-accent rounded-lg border border-accent/20">
                                        {ailment.moves.length}
                                    </span>
                                </h2>
                                <p className="text-text-muted text-sm mt-1">Moves that have a chance to inflict this ailment</p>
                            </div>
                        </div>

                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                            {ailment.moves.map((move) => (
                                <Link
                                    key={move.name}
