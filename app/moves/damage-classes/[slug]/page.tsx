...
export default async function MoveDamageClassDetailPage({ params }: PageProps) {
    const { slug } = await params;
    const damageClass = await getMoveDamageClass(slug);
    const englishName = damageClass.names.find((n) => n.language.name === "en")?.name ?? damageClass.name;
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                        <div>
                            ...
                            <p className="text-text-muted mt-4 max-w-2xl text-lg font-medium leading-relaxed italic">
                                &quot;{damageClass.descriptions.find(d => d.language.name === "en")?.description}&quot;
                            </p>
                        </div>
                    </div>
                </div>
                            ...
                            <div>
                                <h2 className="text-2xl font-black text-text-primary flex items-center gap-3">
                                    ...
                                    <span className="text-sm font-bold px-2 py-0.5 bg-accent/10 text-accent rounded-lg border border-accent/20">
                                        {damageClass.moves.length}
                                    </span>
                                </h2>
                                <p className="text-text-muted text-sm mt-1">Moves that utilize the {englishName.toLowerCase()} calculation system</p>
                            </div>
                        </div>

                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                            {damageClass.moves.map((move) => (
                                <Link
                                    key={move.name}
