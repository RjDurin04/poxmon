import { getEvolutionTrigger } from "@/lib/api";
import Link from "next/link";
import { Zap, ArrowLeft, History } from "lucide-react";
...
export default async function EvolutionTriggerDetailPage({ params }: PageProps) {
    const { slug } = await params;
    const trigger = await getEvolutionTrigger(slug);
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                        <div>
                            ...
                            <p className="text-text-muted mt-4 max-w-2xl text-lg font-medium leading-relaxed italic">
                                The biological or mechanical event that initiates metamorphosis in species across all generations.
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="lg:col-span-12">
                        <div className="flex items-center justify-between mb-8">
                            <div>
                                ...
                                <p className="text-text-muted text-sm mt-1">Species that utilize this trigger for evolution</p>
                            </div>
                        </div>
