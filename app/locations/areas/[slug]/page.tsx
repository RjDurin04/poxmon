...
export default async function LocationAreaDetailPage({ params }: PageProps) {
    const { slug } = await params;
    const area = await getLocationArea(slug);
            ...
            <div className="relative overflow-hidden pt-24 pb-20 px-4 md:px-8 border-b border-border">
                ...
                <div className="relative max-w-7xl mx-auto">
                    ...
                    >
                        ...
                        <span className="text-sm font-black uppercase tracking-widest">Back to {area.location.name.replace(/-/g, " ")}</span>
                    </Link>
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                        <div>
                            ...
                            <p className="text-text-muted mt-4 max-w-2xl text-lg font-medium leading-relaxed italic">
                                Specific biological sub-zone within the {area.location.name.replace(/-/g, " ")} sector.
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="lg:col-span-12">
                        <div className="flex items-center justify-between mb-8">
                            <div>
                                ...
                                <p className="text-text-muted text-sm mt-1">Species documented within this micro-habitat</p>
                            </div>
                        </div>

                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                            {area.pokemon_encounters.map((enc) => {
                                const id = enc.pokemon.url.split("/").filter(Boolean).pop();
