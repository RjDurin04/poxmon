...
export default async function PalParkAreaDetailPage({ params }: PageProps) {
    const { slug } = await params;
    const area = await getPalParkArea(slug);
    const englishName = area.names.find((n) => n.language.name === "en")?.name ?? area.name;
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                        <div>
                            ...
                            <p className="text-text-muted mt-4 max-w-2xl text-lg font-medium leading-relaxed italic">
                                Specialized conservation zone for species migration and historical data preservation.
                            </p>
                        </div>
                    </div>
                </div>
                            ...
                            <div>
                                <h2 className="text-2xl font-black text-text-primary flex items-center gap-3">
                                    ...
                                    <span className="text-sm font-bold px-2 py-0.5 bg-accent/10 text-accent rounded-lg border border-accent/20">
                                        {area.pokemon_encounters.length}
                                    </span>
                                </h2>
                                <p className="text-text-muted text-sm mt-1">Species available for capture in this park sector</p>
                            </div>
                        </div>

                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                            {area.pokemon_encounters.map((enc) => {
                                const id = enc.pokemon_species.url.split("/").filter(Boolean).pop();
                                const spriteUrl = `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/${id}.png`;

                                return (
                                    <Link
                                        key={enc.pokemon_species.name}
                                        href={`/pokemon/${enc.pokemon_species.name}`}
                                    ...
                                    >
                                        ...
                                        <span className="text-[10px] font-black text-text-primary capitalize tracking-tight group-hover:text-accent transition-colors text-center truncate w-full">
                                            {enc.pokemon_species.name.replace(/-/g, " ")}
                                        </span>
                                    </Link>
                                );
                            })}
