...
export function GenerationsClient({ initialGens, totalCount }: { initialGens: NamedAPIResource[], totalCount: number }) {
    ...
    const filteredGens = useMemo(() => {
        return initialGens.filter(gen =>
            gen.name.replace(/-/g, " ").toLowerCase().includes(searchQuery.toLowerCase())
        );
    }, [initialGens, searchQuery]);

    const totalPages = Math.ceil(filteredGens.length / itemsPerPage);
    const paginatedGens = useMemo(() => {
    ...
    return (
        <div className="min-h-screen bg-bg-primary text-text-primary selection:bg-accent/30 selection:text-white pb-20 font-sans">
            ...
            <div className="fixed inset-0 pointer-events-none z-0">
                ...
                <div className="absolute top-1/2 -right-1/4 w-[600px] h-[600px] bg-accent/5 blur-[140px] rounded-full" />
            </div>
                ...
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-12">
                        <div className="relative">
                            ...
                            >
                                Technical listing of game generations and versions according to the PokeAPI database.
                            </motion.p>
                        </div>
                        ...
                        <div className="flex flex-col gap-6 w-full lg:w-auto">
                            <div className="flex items-center gap-6">
                                <DashboardStat label="Total Records" value={totalCount} icon={Gamepad2} />
            ...
            <main className="relative z-10 max-w-7xl mx-auto px-4 md:px-8 py-20">
                <AnimatePresence mode="wait">
                    ...
                    >
                        {paginatedGens.map((gen) => {
                            const id = gen.url.split("/").filter(Boolean).pop();
                            return (
                                <GameCard key={gen.name} gen={gen} id={id || "0"} />
                            );
                        })}
                    </motion.div>
                </AnimatePresence>
                ...
                {filteredGens.length === 0 && (
                    ...
                    >
                        ...
                        <h3 className="text-2xl font-black text-text-secondary uppercase tracking-widest">No generation data found</h3>
                    </motion.div>
                )}
                    ...
                    <div className="mt-20 flex flex-col sm:flex-row items-center justify-between gap-8 border-t border-border/50 pt-12">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex items-baseline gap-1">
                                ...
                                <span className="text-xl font-black text-accent">{filteredGens.length}</span>
                            </div>
                        </div>
...
function GameCard({ gen, id }: { gen: NamedAPIResource, id: string }) {
    return (
        <motion.div
            layout
            variants={{
            ...
            className="group"
        >
            <Link href={`/games/${gen.name}`} className="block relative h-full">
                    ...
                    <div className="relative z-10 flex flex-col h-full min-h-[110px]">
                        <div className="flex items-center justify-between mb-8">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-[9px] font-mono text-accent font-black tracking-widest">GEN-{id.padStart(2, '0')}</span>
                            </div>
                            <ArrowUpRight className="w-4 h-4 text-text-muted opacity-0 group-hover:opacity-100 transition-all font-black" />
                        </div>

                        <h2 className="text-xl font-black text-text-primary uppercase tracking-tighter leading-tight group-hover:text-accent transition-colors">
                            {gen.name.replace("generation-", "GEN ")}
                        </h2>

                        <div className="mt-auto pt-4 flex items-center justify-between">
                            <span className="text-[9px] font-black text-text-muted uppercase tracking-widest">Data Record</span>
