...
interface BerriesClientProps {
    initialBerries: NamedAPIResource[];
    totalCount: number;
}

export function BerriesClient({ initialBerries, totalCount }: BerriesClientProps) {
    ...
    const filteredBerries = useMemo(() => {
        return initialBerries.filter(berry =>
            berry.name.replace(/-/g, " ").toLowerCase().includes(searchQuery.toLowerCase())
        );
    }, [initialBerries, searchQuery]);

    const totalPages = Math.ceil(filteredBerries.length / itemsPerPage);
    ...
    return (
        <div className="min-h-screen bg-bg-primary text-text-primary selection:bg-accent/30 selection:text-white pb-20 font-sans">
            ...
            <div className="fixed inset-0 pointer-events-none z-0">
                ...
                <div className="absolute top-1/4 -right-1/4 w-[600px] h-[600px] bg-emerald-500/5 blur-[120px] rounded-full" />
            </div>
            ...
            <header className="relative z-10 pt-32 sm:pt-44 lg:pt-52 pb-16 px-4 md:px-8 border-b border-border/50 bg-bg-primary/50 backdrop-blur-xl">
                <div className="max-w-7xl mx-auto">
                    ...
                    >
                        <div className="w-12 h-12 rounded-2xl bg-emerald-500 flex items-center justify-center p-2 shadow-[0_0_30px_rgba(16,185,129,0.3)]">
                            <FlaskConical className="w-6 h-6 text-white" />
                        </div>
                        <div className="flex flex-col">
                            <span className="text-[10px] font-black uppercase tracking-[0.4em] text-emerald-500 leading-none mb-1.5">Berry Registry</span>
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-12">
                        <div className="relative">
                            ...
                            >
                                Berry<br />
                                <span className="text-emerald-500 underline decoration-4 underline-offset-8 decoration-emerald-500/20">Directory</span>
                            </motion.h1>
                            <motion.p
                                initial={{ opacity: 0 }}
                                animate={{ opacity: 1 }}
                            ...
                            >
                                Comprehensive catalog of all known Pokémon berries. Data includes names and identifiers synced directly with the API.
                            </motion.p>
                        </div>
                        ...
                        <div className="flex flex-col gap-6 w-full lg:w-auto">
                            <div className="flex items-center gap-6">
                                <DashboardStat label="Total Records" value={totalCount} icon={Leaf} />
            ...
            <main className="relative z-10 max-w-7xl mx-auto px-4 md:px-8 py-20">
                <AnimatePresence mode="wait">
                    ...
                    >
                        {paginatedBerries.map((berry) => {
                            const id = berry.url.split("/").filter(Boolean).pop();
                            return (
                                <BerryCard key={berry.name} berry={berry} id={id || "0"} />
                            );
                        })}
                    </motion.div>
                </AnimatePresence>
                ...
                {filteredBerries.length === 0 && (
                    ...
                    >
                        ...
                        <h3 className="text-2xl font-black text-text-secondary uppercase tracking-widest">No specimen identified</h3>
                    </motion.div>
                )}
                ...
                {totalPages > 1 && (
                    <div className="mt-20 flex flex-col sm:flex-row items-center justify-between gap-8 border-t border-border/50 pt-12">
                        <div className="flex items-center gap-4">
                            <span className="text-[10px] font-black text-text-muted uppercase tracking-[0.3em]">Registry</span>
                            <div className="flex items-baseline gap-1">
                                ...
                                <span className="text-xl font-black text-text-primary">{Math.min(currentPage * itemsPerPage, filteredBerries.length)}</span>
                            </div>
                            <span className="text-[10px] font-black text-text-muted uppercase tracking-[0.3em]">OF</span>
                            <span className="text-xl font-black text-emerald-500">{filteredBerries.length}</span>
                        </div>
        ...
        <button
            ...
            className={cn(
                ...
                disabled
                    ...
                    : "bg-bg-secondary text-text-primary hover:border-emerald-500/40 hover:bg-bg-tertiary shadow-sm"
            )}
        >
...
function BerryCard({ berry, id }: { berry: NamedAPIResource, id: string }) {
    return (
        <motion.div
            layout
            variants={{
            ...
            className="group"
        >
            <Link href={`/berries/${berry.name}`} className="block relative h-full">
                <div className="relative h-full bg-bg-secondary border border-border/60 rounded-[28px] p-6 transition-all duration-300 group-hover:border-emerald-500/40 group-hover:bg-bg-tertiary shadow-sm overflow-hidden">
                    <div className="absolute top-0 right-0 p-6 opacity-[0.03] group-hover:opacity-[0.08] transition-opacity">
                        <Sprout className="w-20 h-20 text-emerald-500" />
                    </div>

                    <div className="relative z-10 flex flex-col h-full min-h-[110px]">
                        <div className="flex items-center justify-between mb-8">
                            <div className="flex items-center gap-2">
                                <div className="p-2 rounded-lg bg-bg-primary border border-border group-hover:bg-emerald-500 group-hover:text-white transition-colors">
                                    <Leaf className="w-4 h-4" />
                                </div>
                                <span className="text-[9px] font-mono text-emerald-500 font-black tracking-widest">BRY-{id.padStart(4, '0')}</span>
                            </div>
                            <ArrowUpRight className="w-4 h-4 text-text-muted opacity-0 group-hover:opacity-100 transition-all font-black" />
                        </div>

                        <h2 className="text-xl font-black text-text-primary capitalize tracking-tighter leading-tight group-hover:text-emerald-500 transition-colors">
                            {berry.name.replace(/-/g, " ")}
                        </h2>

                        <div className="mt-auto pt-4 flex items-center justify-between">
                            <span className="text-[9px] font-black text-text-muted uppercase tracking-widest">API Record</span>
                            <div className="h-0.5 w-6 bg-border group-hover:w-12 group-hover:bg-emerald-500 transition-all" />
                        </div>
                    </div>
                </div>
            </Link>
